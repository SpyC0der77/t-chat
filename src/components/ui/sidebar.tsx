...
const MIN_SIDEBAR_WIDTH = "256px"
const MAX_SIDEBAR_WIDTH = "864px"
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("size-8 z-10", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
...
function SidebarRail({ className, ...props }: React.ComponentProps<"div">) {
  ...
  return (
    <div
      ...
      className={cn("absolute right-0 top-0 h-full w-2 cursor-col-resize", className)}
      {...props}
    />
  )
}
